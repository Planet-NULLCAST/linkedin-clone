...
const Post = () => {
  ...
  const { profilePicUrl, userName, id } = useContext(LoginContext);
  ...
  const addphotoTopost = async () => {
    setcreatePost(true);
    setselectImage(false);
    const formData = new FormData();
    formData.append("postImage", file, fileName);

    const res = await fetch(`${SERVER_URL}/posts/image/${id}`, {
      method: "POST",
      body: formData,
      headers: {
  ...
  const handlePostSubmit = async () => {
    if (!url && !inputfield) {
      ...
      const response = await fetch(`${SERVER_URL}/posts`, {
        method: "POST",
        body: JSON.stringify({text:inputfield,imageUrl,createdById:id}),
        headers : {
          "Authorization" : "Bearer" + " " + token,
  ...
  return (
    <div>
      ...
      {createPost ? (
        <div className="absolute -top-0 -left-0  flex justify-center  w-full h-screen bg-black bg-opacity-40">
                ...
                {url ? (
                  <div className="w-99 h-60 overflow-scroll border-none p-4 z-30">
                    <div className="flex">
                      ...
                      ></img>
                      <div>
                        <label className="text-black-500 text-base">
                          {userName}
                        </label>
                        <div className="border justify-evenly border-black opacity-75 rounded-full flex w-full items-center">
                          <button className="flex items-center ">
            ...
            <div className="items-center justify-start pl-4 py-1 flex bg-white rounded shadow">
              ...
              <div className="flex items-center px-7 w-full justify-between border-l-2">
                <div className="hover:bg-gray-300  w-17 h-5 rounded-full flex items-center justify-center mr-13">
                  <button className="flex justify-center items-center  text-xs">
                    <i className="far fa-comment-dots  text-xl opacity-50"></i>
                    Anyone
                  </button>
                </div>
                <div
                  className={`bg-blue-500 text-white ml-1 w-16 h-8 flex justify-center rounded-full items-center`}
                >
                  <button className="bg-blue-500" onClick={handlePostSubmit}>
                    post
                  </button>
                </div>
              </div>
        ...
        <div className="absolute inset-0 -top-0 -left-0 flex justify-center  w-full h-screen bg-black bg-opacity-40">
          {" "}
          <div className="z-30 shadow rounded bg-white mt-8 h-20">
